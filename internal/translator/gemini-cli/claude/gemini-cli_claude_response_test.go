...
func TestConvertGeminiCLIResponseToClaude_StreamMessageStartUsesGPTOriginalRequestFallback(t *testing.T) {
	...
	rawJSON := []byte(`{"response":{"responseId":"resp_123","modelVersion":"gemini-2.5-pro","candidates":[{"content":{"parts":[{"text":"hello"}]}}]}}`)

	var param any
	outputs := ConvertGeminiCLIResponseToClaude(ctx, "", originalRequest, requestRawJSON, rawJSON, &param)
